# Version of popular board-leading kernel that exorcises a magic number of
#   possibly rule-breaking origin

# Follow the fork back for details.

# I've confirmed that changing the macro humility factor from the old over-precise value
#   to a nice round .7 does not affect the reported leaderboard score


# I'm adding Bill S's changes now
...
macro_humility_factor = 0.7  # I contend this is a legitimate tunable parameter,
#    not a magic number
#  You can re-run the model on a later data set and
#    use the same value
#  and that would be a perfecly reasonable thing to do,
#    unlike reusing an arbitrary number

#  (Of course, there are magic numbers elsewhere in this script,
#   but they're not derived from ground-truth-related data,
#   so they aren't suspect of violating competition rules.
#   Had we but world enough, and time,
#   I expect I could find ways to get rid of those numbers too.)

# The concept of "humility factor" is that the substantive macro model will be overfit,
#   so you average in a naive macro model result as a form of regularization.

# You can tune it by using the public leaderboard as a cross-validation set
#   but we already know this value works well.
...
print("Modeled macro mean: " + str(macro_mean))
...
#
# train['car_transport_km']  = train['metro_km_avto'] * train['bus_terminal_avto_km']
# test['car_transport_km']  = test['metro_km_avto'] * test['bus_terminal_avto_km']
#
# train['car_transport2_km']  = train['railroad_station_avto_km'] * train['zd_vokzaly_avto_km']
# test['car_transport2_km']  = test['railroad_station_avto_km'] * test['zd_vokzaly_avto_km']
#
# train['access_to_work']  = train['workplaces_km'] * train['office_km']
# test['access_to_work']  =  test['workplaces_km'] * test['office_km']
...
# train.ix[bad_index, "build_year"] = 2000
...
#
# train['floor'] = train['floor'].fillna(round(train['floor'].median()))
# test['floor'] = test['floor'].fillna(round(test['floor'].median()))
...
lnmicro = pd.Series(y_predict).apply(np.log).mean()
micro_mean = np.exp(lnmicro)
print("Raw micro mean: " + str(micro_mean))
...
train = pd.read_csv('../input/train_fill.csv')
test = pd.read_csv('../input/test_fill.csv')
id_test = test.id
...
df_all['pts_floor_ratio'] = df_all['public_transport_station_km'] / df_all['max_floor'].astype(
    float)  # apartments near public t?
...
df_all['pts_x_state'] = df_all['public_transport_station_km'] * df_all['state'].astype(
    float)  # public trans * state of listing
...
newpreds.to_csv('final_adjusted_result.csv', index=False)