...
if is_valid:
    ...
    num_round = 15
    model = xgb.train(params, dtrain, num_round, watchlist, maximize=True, verbose_eval=1)
...
submission_file_name = 'submission_xgboost_baseline_eta_' + str(params['eta']) + '_round_' + str(num_round) + '_' + str(datetime.datetime.now().strftime("%Y_%m_%d_%H_%M")) + '.csv'
