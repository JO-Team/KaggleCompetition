...
def read_parse_dataframe(file_name):
    ...
    for col in cols_to_parse:
        ...
        json_col_df.columns = ["{col}_{sub_col}".format(col=col, sub_col=sub_col) for sub_col in json_col_df.columns]
...
lgb_params1 = {"objective": "regression", "metric": "rmse", "max_depth": 8, "min_child_samples": 20, "reg_alpha": 1,
               "reg_lambda": 1,
...
if run_lgb:
    ...
    def kfold_lgb_xgb():
        ...
        for fold_, (trn_idx, val_idx) in enumerate(folds.split(X)):
            ...
            num_round = 20000
            clf = lgb.train(lgb_params1, trn_data, num_round, valid_sets=[trn_data, val_data], verbose_eval=1000,
        ...
        cols = feature_importance_df_lgb[["feature", "importance"]].groupby("feature").mean().sort_values(
