...
train2 = pd.read_csv('../input/train_v2.csv')
...
def make_transactions_v2_features():
    print('loading...')
    infos = {}
...
def make_userlog_features():
    ...
    with open('../input/user_logs.csv') as fd:
        ...
        for line in fd:
            pos = line.find(',')
            msid = line[:pos]
            # _, num_25, num_50, num_75, num_985, num_100, num_unq, total_secs =
            # [int(float(value)) for value in line[pos + 1:-1].split(',')]
            splits = line[pos + 1:-1].split(',')
...
transactions_v2 = make_transactions_v2_features()
user_logs_v2 = make_userlog_v2_features()

train = pd.merge(train, transactions, how='left', on='msno')
test = pd.merge(test, transactions, how='left', on='msno')

train = pd.merge(train, transactions_v2, how='left', on='msno')
test = pd.merge(test, transactions_v2, how='left', on='msno')

train = pd.merge(train, user_logs, how='left', on='msno')
test = pd.merge(test, user_logs, how='left', on='msno')

train = pd.merge(train, user_logs_v2, how='left', on='msno')
test = pd.merge(test, user_logs_v2, how='left', on='msno')
...
train = pd.merge(train, members_v2, how='left', on='msno')
test = pd.merge(test, members_v2, how='left', on='msno')
...
# test['is_churn'] = test['is_churn'].map(lambda x: 0.0 if x < 0.001 else x)
test[['msno', 'is_churn']].to_csv('submission_baseline_xgboost_merged_data.csv', index=False)
